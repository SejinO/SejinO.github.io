/*
https://www.openprocessing.org/sketch/765915
Get the code from "A fork of Tree P5 js Version by Florian rey."
*/
var myTree;
var startPoint;
var direction;
...
function draw() 
{
  ...
  for(i = 0; i < myTree.twig.length; i++)
  {
    var num = myTree.twig[i].location.length - 1;
		// stroke(0);
		var distance = dist(mouseX, mouseY, myTree.twig[i].location[num].x, myTree.twig[i].location[num].y);
		maxD = distance > maxD ? distance : maxD;
		// if(distance < 20){
		// 	line(mouseX, mouseY, myTree.twig[i].location[num].x, myTree.twig[i].location[num].y);
		// }
		// noStroke();
		
		// var d = myTree.twig[i].location[num].copy();
		// d.sub(mouseX, mouseY);
		// d.normalize();
		// fill(255 * (1.0 - d.x), 75);
		colorMode(HSB);
		var flower = noise(3 + i);
		fill(floor(flower * 300), floor(flower * 120), floor(flower * 120), 100);
		// if (flower < 0.3){
			ellipse(myTree.twig[i].location[num].x, myTree.twig[i].location[num].y, 3 * log(abs((distance/maxD))), 3 * log(abs((distance/maxD))));
		// }
  }
}
...
Frontier.prototype.update = function(factor) 
{
     ...
     & this.thickness > factor)  
  {
    ...
    this.velocity.mult(random(8, 30));
